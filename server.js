...
mongoose.connect(
  process.env.MLAB_URI ||
    "mongodb+srv://abhishek:4otQTbFYlZSM3pBV@cluster0-nwvsc.mongodb.net/exercise-track-db?retryWrites=true&w=majority"
);
...
app.get("/api/exercise/log", async (req, res) => {
  try {
    ...
    if (userExist) {
      result["_id"] = userExist._id;
      result["username"] = userExist.username;

      /*result["user"] = [];
      result.user.push({ _id: userExist._id, username: userExist.username });*/
...
/*app.get("/api/exercise/log/:userId", async (req, res) => {
  try {
    ...
    if (userExist) {
      result["_id"] = userExist._id;
      result["username"] = userExist.username;
      var exercises = await exercises_model
